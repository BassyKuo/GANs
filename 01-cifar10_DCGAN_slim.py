...
model_name = '01-cifar10_DCGAN_slim'
summary_dir = './summary/'
...
f_dim = [1024, 256, 128, img_channel]
...
def trainGAN(checkpoint=None, model_name=model_name):
    ...
    images = cifar10.train.data.reshape(-1, 32, 32, 3)
    labels = cifar10.train.labels.reshape(-1, 10)
    # train = defaultdict(lambda: [])
    # for i,c in enumerate(labels):
    #     train[c.argmax()].append(images[i])
    # for i in train.keys():
    #     train[i] = np.array(train[i])
    #     train[i] = train[i] * 2. - 1.   # CENTRAL
    # print ('Using cifar10 Data')
    # print ('[!] train shape: ', [train[k].shape for k in train.keys()])
    # train = train[N]
    train = (images * 2.) - 1.
