...
model_name = '01-mnist_DCGAN_slim'
summary_dir = './summary/'
...
args.DEFINE_integer('max_epoch',10000,  '[%(default)s]')
...
def generator(z, batch_size=batch_size, phase_train=True, reuse=False, use_bias=False, name="gen"):
    ...
    weight_init = tf.truncated_normal_initializer(stddev=0.1)  # Recommanded to train NN

    with tf.variable_scope(name, reuse=reuse):
        # FIRST LAYER
        net = slim.fully_connected(z, 7*7*128, activation_fn=None, weights_initializer=weight_init, trainable=phase_train)
        net = tf.reshape(net, [batch_size, 7, 7, 128])
        print ("G0: ", net)

        # HIDDEN LAYER
        net = slim.conv2d_transpose(net, 64, (4,4), stride=2, padding='SAME', weights_initializer=weight_init, trainable=phase_train, activation_fn=None)
        # net = slim.batch_norm(net, epsilon=1e-5, trainable=phase_train, fused=True)
        net = tf.nn.relu(net)
        print ("G1: ", net)

        net = slim.conv2d_transpose(net, 32, (4,4), stride=2, padding='SAME', weights_initializer=weight_init, trainable=phase_train, activation_fn=None)
        # net = slim.batch_norm(net, epsilon=1e-5, trainable=phase_train, fused=True)
        net = tf.nn.relu(net)
        print ("G2: ", net)
...
def discriminator(inputs, phase_train=True, reuse=False, use_bias=False, name="dis"):
    ...
    with tf.variable_scope(name, reuse=reuse):
        # FIRST LAYER
        net = slim.conv2d(inputs, 32, (4,4), stride=1, padding='SAME', weights_initializer=weight_init, trainable=phase_train, activation_fn=tf.nn.relu)
        print ("D0: ", net)

        # HIDDEN LAYER
        net = slim.conv2d(net, 64, (4,4), stride=2, padding='SAME', weights_initializer=weight_init, trainable=phase_train, activation_fn=None)
        # net = slim.batch_norm(net, epsilon=1e-5, trainable=phase_train, fused=True)
        net = tf.nn.relu(net)
        feature_map = net
        print ("D1: ", net)

        net = slim.conv2d(net, 128, (4,4), stride=2, padding='SAME', weights_initializer=weight_init, trainable=phase_train, activation_fn=None)
        # net = slim.batch_norm(net, epsilon=1e-5, trainable=phase_train, fused=True)
        net = tf.nn.relu(net)
        print ("D2: ", net)
